...
num_x = int((int((domain_x-tol) / (2 * radius + tol)))/2)
num_y = int((domain_y-tol) / (2 * radius + tol))
for k in range (0,15,1):
    for j in range(0, num_y, 1):
        for i in range(0, num_x, 1):
            pos = []
            if k%2 == 0:
                pos.append(-domain_x/2+tol/2+ radius + i*(radius*2+tol))
            else:
                pos.append(-domain_x/2+tol/2+ radius + i*(radius*2+tol)+tol)
                
            if k%2 == 0:
                pos.append(-domain_y/2+tol/2 + radius + j*(radius*2+tol))
            else:
                pos.append(-domain_y/2+tol/2+ radius + j*(radius*2+tol)+tol)
...
for i in range(10000):
  ...
  dem1.forward(0.0005)
  if i%write_skip == 0:
      dem1.csv_output("fld_db_ml/test"+str(write_count))
